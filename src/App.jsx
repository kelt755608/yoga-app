      ...
      <main className="mx-auto grid max-w-6xl gap-4 px-4 py-4 lg:grid-cols-[360px_minmax(0,1fr)]">
        ...
        <section className="space-y-4 rounded-2xl border border-slate-800 bg-slate-900/70 p-4 shadow-lg shadow-slate-950/60">
            ...
            <div className="mt-3 space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <div className="space-y-1">
                  ...
                  <input
                    className="w-full rounded-lg border border-slate-700 bg-slate-900/80 px-2 py-1.5 text-xs outline-none focus:border-emerald-400 focus:ring-1 focus:ring-emerald-500/50"
                    value={privateClassTarget}
                    onChange={(e) => setPrivateClassTarget(e.target.value)}
                  />
                </div>
              </div>
            </div>
          ...
          <button
            ...
            className="mt-3 inline-flex w-full items-center justify-center gap-2 rounded-xl bg-emerald-500 px-3 py-2 text-xs font-semibold text-slate-950 shadow-lg shadow-emerald-500/40 hover:bg-emerald-400 active:scale-[0.99]"
          >
        ...
        <section className="space-y-4">
          ...
          <div className="grid gap-4 lg:grid-cols-2">
            {/* 门店信息脚本 */}
            <div className="flex flex-col rounded-2xl border border-slate-800 bg-slate-900/70 p-3 shadow-md shadow-slate-950/60">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <div>
                    ...
                    <div className="text-[11px] text-slate-400">
                      用于方案/PPT 开头、当面沟通时的介绍段
                    </div>
                  </div>
                </div>
                <button
                  type="button"
                  onClick={() => handleCopy(storeScript)}
                  className="inline-flex items-center gap-1 rounded-full border border-slate-700 px-2 py-0.5 text-[11px] text-slate-300 hover:border-emerald-400 hover:text-emerald-200"
                >
              ...
              <textarea
                className="custom-scrollbar flex-1 rounded-xl border border-slate-800 bg-slate-950/60 p-2 text-[11px] leading-relaxed text-slate-200"
                rows={10}
                value={storeScript}
                onChange={(e) => setStoreScript(e.target.value)}
                placeholder="点击左侧按钮生成脚本…"
              />
            </div>
            ...
            <div className="flex flex-col rounded-2xl border border-slate-800 bg-slate-900/70 p-3 shadow-md shadow-slate-950/60">
              <div className="mb-2 flex items-center justify-between gap-2">
                ...
                <button
                  type="button"
                  onClick={() => handleCopy(momentScript)}
                  className="inline-flex items-center gap-1 rounded-full border border-slate-700 px-2 py-0.5 text-[11px] text-slate-300 hover:border-emerald-400 hover:text-emerald-200"
                >
                  <Copy className="h-3 w-3" />
                  复制
              ...
              <textarea
                ...
                rows={10}
                value={momentScript}
                onChange={(e) => setMomentScript(e.target.value)}
                placeholder="点击左侧按钮生成脚本…"
              />
            </div>
            ...
            <div className="flex flex-col rounded-2xl border border-slate-800 bg-slate-900/70 p-3 shadow-md shadow-slate-950/60">
              <div className="mb-2 flex items-center justify-between gap-2">
                ...
                <button
                  type="button"
                  onClick={() => handleCopy(salesScript)}
                  className="inline-flex items-center gap-1 rounded-full border border-slate-700 px-2 py-0.5 text-[11px] text-slate-300 hover:border-emerald-400 hover:text-emerald-200"
                >
                  <Copy className="h-3 w-3" />
                  复制
          ...
          {/* AI 问答区 */}
          <div className="rounded-2xl border border-slate-800 bg-slate-900/70 p-3 shadow-md shadow-slate-950/60">
            <div className="mb-2 flex items-center justify-between gap-2">
              <div className="flex items-center gap-2">
                <Sparkles className="h-4 w-4 text-emerald-400" />
                <div>
            ...
            <div className="flex flex-col gap-2 md:flex-row">
              <input
                className="flex-1 rounded-xl border border-slate-700 bg-slate-950/80 px-3 py-2 text-xs outline-none focus:border-emerald-400 focus:ring-1 focus:ring-emerald-500/50"
                placeholder="例如：帮我优化一个适合上班族的晚间团课排期 / 帮我设计一个老客户续费活动…"
                value={qaInput}
              ...
              <button
                ...
                className="inline-flex items-center justify-center gap-2 rounded-xl bg-emerald-500 px-3 py-2 text-xs font-semibold text-slate-950 disabled:cursor-not-allowed disabled:bg-slate-700"
              >
                <Sparkles className="h-4 w-4" />
                {isQaLoading ? "AI 正在思考…" : "向强中强 AI 提问"}
              </button>
            </div>

            <div className="mt-3 max-h-60 space-y-2 overflow-y-auto custom-scrollbar text-[11px] leading-relaxed">
              {qaHistory.length === 0 && (
                <p className="text-slate-400">
                  你可以这样问：
                  <br />
                  · “帮我写一个 12 月冲刺业绩的三段式朋友圈文案”
                  <br />
                  · “如何把体验课转化率从 20% 提升到 40%？”
                </p>
              )}
              ...
              {qaHistory.map((item, idx) => (
                ...
                >
                  ...
                  <div className="mt-1 whitespace-pre-wrap text-[11px] text-slate-300">
                    {item.answer}
                  </div>
                </div>
              ))}
